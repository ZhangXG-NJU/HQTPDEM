...
# 读取CSV文件
df = pd.read_csv(r"E:\Zhang_XinGang\11_QPTDEM\03_data\atl08\08_pquery\03_MinMaxAdd.csv")
df = shuffle(df,random_state=42).iloc[:, :]
...
rf = RandomForestRegressor(max_depth=None,
                           ...
                           min_samples_split=2, 
                           min_samples_leaf=1,
                           max_features = "log2",  #0415测试，需要log2
                           bootstrap=False,
                           n_jobs=-1)
...
# 输出结果
print(f"Best Params: {rf.get_params()}, MAE: {mae}, RMSE: {rmse}, MSE: {mse}")
...
def getSeries(x, y):
    ...
    _, Ysize, Xsize  = ReadGeoTiff(f"../../03_data/01_aster/dem_sub/aster_dem_y{y}_x{x}.tif")
    GeoT, Projection = GetGeoInfo(f"../../03_data/01_aster/dem_sub/aster_dem_y{y}_x{x}.tif")
    ...
    for x in range(1,14):
        for y in range(1,7):
            try:
                ...
                Xs_pred  = rf.predict(XsArray).reshape((Ysize, Xsize))
                filename = f"RF_ATL08_y{y}x{x}"
